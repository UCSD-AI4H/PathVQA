...
class VisualConfig(object):
    ...
    def __init__(self,
        ...
        self.l_add_layers = l_add_layers
...
class LXRTEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.num_l_add_layers = VISUAL_CONFIG.l_add_layers
        self.num_x_layers = VISUAL_CONFIG.x_layers
        self.num_r_layers = VISUAL_CONFIG.r_layers
        print("LXRT encoder with %d l_layers, %d x_layers, and %d r_layers. %d additional l_layers" %
              (self.num_l_layers, self.num_x_layers, self.num_r_layers, self.num_l_add_layers))
        ...
        self.l_add_layers = nn.ModuleList(
            [BertLayer(config) for _ in range(self.num_l_add_layers)]
        )

    def forward(self, lang_feats, lang_attention_mask,
                visn_feats, visn_attention_mask=None, t='vqa'):
        ...
        # Run Additional Language layers
        if t == 'qa_woi':
            for layer_module in self.l_add_layers:
                lang_feats = layer_module(lang_feats, lang_attention_mask)
...
class BertQuestionAnswerHead(nn.Module):
    def __init__(self, config, num_answers):
        super().__init__()
        hid_dim = config.hidden_size
        self.logit_fc = nn.Sequential(
...
class VisualAnswerHead(nn.Module):
    def __init__(self, config, num_answers):
        super().__init__()
        hid_dim = config.hidden_size
        self.logit_fc = nn.Sequential(
...
class LXRTModel(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.pooler_qa_woi = BertPooler(config)
        self.pooler_va2 = BertPooler(config)
        self.apply(self.init_bert_weights)

    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
                visual_feats=None, visual_attention_mask=None, t='vqa'):
        # t in ('vqa', 'va', 'qa_woi')
        ...
        if t == 'qa_woi':
            ...
            pooled_output = self.pooler_va2(visn_feats)
            # print('va2: pooled_output.shape = ',pooled_output.shape)
        else:
...
class LXRTPretraining(BertPreTrainedModel):
    def __init__(self,
        ...
        self.task_qa_woi = task_qa_woi
        self.task_va = task_va
        self.task_va2 = task_va2
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None,
                ...
                matched_label=None, matched_label_ans=None,
                ans=None, replace_ans=None,
                ans_types=None, ans_rps_types=None,
                input_ids_rps=None, token_type_ids_rps=None, attention_mask_rps=None, masked_lm_labels_rps=None,
        ...
        if args.task_qa_woi:
            ...
            # lang_prediction_scores_qa_woi, cross_relationship_score_qa_woi = self.cls(
        ...
        if self.task_va2:
            ...
            pass  # answer_score_va2 = pooled_output[0][0]
        ...
        if ans is not None and self.task_qa_woi:
            answer_loss_qa_woi = loss_fct(
